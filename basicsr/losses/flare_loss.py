...
@LOSS_REGISTRY.register()
class L_frequency(nn.Module):
    def __init__(self, loss_weight=1.0):
        super(L_frequency, self).__init__()
        self.loss_weight = loss_weight
        self.loss = FFL(loss_weight=1.0, alpha=1.0)

    def forward(self, fake, target):
        frequency_loss = self.loss(fake, target)
        return self.loss_weight * frequency_loss
...
class WeightedBCE(nn.Module):
    ...
    def forward(self, input, target):
        ...
        bce = - self.class_weight[1] * target * torch.log(input) - (1 - target) * self.class_weight[0] * torch.log(
            1 - input)
