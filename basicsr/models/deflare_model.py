...
class DeflareModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        self.l_frequency = build_loss(train_opt['frequency']).to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, self.pred_flare = self.net_g(self.lq)
        ...
        if self.output_ch == 6:
            ...
            self.flare_hat = self.pred_flare
        else:
        ...
        # frequency loss
        l_frequency = self.l_frequency(self.deflare, self.gt)
        l_total += l_frequency
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output, self.pred_flare = self.net_g_ema(self.lq)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output, self.pred_flare = self.net_g(self.lq)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            fl_img = tensor2img([visuals['flare']])
            ...
            flare_name = 'flare' + str(idx).zfill(5) + '_'
            if save_img:
                if self.opt['is_train']:
                    save_img_path = osp.join(self.opt['path']['visualization'], img_name,
                                             f'{img_name}_{current_iter}.png')
                    save_flare_path = osp.join(self.opt['path']['visualization'], flare_name,
                                             f'{flare_name}_{current_iter}.png')
                else:
                    if self.opt['val']['suffix']:
                        save_img_path = osp.join(self.opt['path']['visualization'], dataset_name + '/deflare',
                                                 f'{img_name}_{self.opt["val"]["suffix"]}.png')
                        save_flare_path = osp.join(self.opt['path']['visualization'], dataset_name + '/flare',
                                                 f'{flare_name}_{self.opt["val"]["suffix"]}.png')
                    else:
                        save_img_path = osp.join(self.opt['path']['visualization'], dataset_name + '/deflare',
                ...
                imwrite(fl_img, save_flare_path)
