...
class Uformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        # Input Projection
        y = self.input_proj(x)
        y = self.pos_drop(y)  # 1*262144*32
        # Encoder
        conv0 = self.encoderlayer_0(y, mask=mask)  # 1*262144*32
        pool0 = self.dowsample_0(conv0)  # 1*65536*64
        conv1 = self.encoderlayer_1(pool0, mask=mask)  # 1*65536*64
        pool1 = self.dowsample_1(conv1)  # 1*16384*128
        conv2 = self.encoderlayer_2(pool1, mask=mask)  # 1*16384*128
        pool2 = self.dowsample_2(conv2)  # 1*4096*256
        conv3 = self.encoderlayer_3(pool2, mask=mask)  # 1*4096*256
        pool3 = self.dowsample_3(conv3)  # 1*1024*512

        # Bottleneck
        conv4 = self.conv(pool3, mask=mask)  # 1*1024*512

        # Decoder
        up0 = self.upsample_0(conv4)  # 1*4096*256
        deconv0 = torch.cat([up0, conv3], -1)  # 1*4096*512
        deconv0 = self.decoderlayer_0(deconv0, mask=mask)  # 1*4096*512

        up1 = self.upsample_1(deconv0)  # 1*16384*128
        deconv1 = torch.cat([up1, conv2], -1)  # 1*16384*256
        deconv1 = self.decoderlayer_1(deconv1, mask=mask)  # 1*16384*256
        ...
        deconv3 = self.decoderlayer_3(deconv3, mask=mask)  # 1*262144*64

        # Output Projection
        y = self.output_proj(deconv3)  # 1*6*512*512
        y = self.activation(y)
        return y
